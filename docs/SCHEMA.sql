...
-- ---------------------------------------------------------------------------
-- TATUM WEBHOOK LOGS
-- Stores every inbound Tatum webhook for debugging and idempotency.
-- UNIQUE on tatum_tx_id prevents double-processing of retried webhooks.
-- ---------------------------------------------------------------------------

create table if not exists public.tatum_webhook_logs (
  id                  uuid primary key default gen_random_uuid(),
  tatum_tx_id         text not null,
  chain               text not null,
  ...
  processing_status   text not null default 'received',
    -- received | matched | approved | skipped | error
  error_message       text,
  created_at          timestamptz not null default now()
);

create unique index if not exists idx_tatum_webhook_logs_tatum_tx_id
  on public.tatum_webhook_logs(tatum_tx_id);

create index if not exists idx_tatum_webhook_logs_created_at
  on public.tatum_webhook_logs(created_at desc);
