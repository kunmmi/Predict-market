...
import { DiceRollSchema } from "@/lib/validations/dice";
...
export async function POST(req: Request) {
  ...
  if (!await rateLimit(`dice:${profileId}`, 30, 60_000)) {
  ...
  const { data, error } = await admin.rpc("resolve_dice_round", {
  ...
  if (error) {
    ...
    console.error("[dice/roll] RPC error:", error);
