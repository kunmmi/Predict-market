-- =============================================================================
-- Migration: Lower tier-2 promoter commission rate from 1.5% → 1%
--
-- Tier 2 = indirect referral (the promoter who referred the tier-1 promoter).
-- Previously 0.015 (1.5% of the trading fee), now 0.01 (1% of the trading fee).
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Replace place_trade — update the hardcoded v_tier2_rate constant
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_tier2_rate constant numeric := 0.01;  -- 1% of the trading fee
begin
  if p_amount <= 0 then
...
-- ---------------------------------------------------------------------------
-- 2. Update the comment on the tier column to reflect new rate
-- ---------------------------------------------------------------------------
comment on column public.commissions.tier is
  '1 = direct referral commission (promoter rate % of fee), 2 = indirect referral commission (1% of fee)';
